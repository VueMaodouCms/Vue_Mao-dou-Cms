    ...
    sortedData () {
      ...
      sortedData.forEach(perData => {
        ...
        if (perData.children !== undefined) {
          ...
          perData.page = 0
          perData.surface = {
            transform: `rotateY(0deg) translateZ(${distnce + 10}px)`,
            background: this.backgroundColor
          }
          let i = 0
          perData.children.forEach(child => {
            i++
            child.surface = { transform: `rotateY(${360 / (perData.children.length + 1) * (i)}deg) translateZ(${distnce + 10}px)`, background: this.backgroundColor }
